...
# Loghi ufficiali dei social (path SVG, fonte: simple-icons). Monocromo:
# ereditano il colore del testo via fill="currentColor".
_SOCIAL_SVG = {
...
def _social_icon_svg(name: str, size: float = 18) -> str:
    path = _SOCIAL_SVG.get(name)
    if not path:
        return ""
    return (
        f'<svg viewBox="0 0 24 24" width="{size}" height="{size}" '
        f'fill="currentColor" style="flex-shrink:0" aria-hidden="true">'
        f'<path d="{path}"/></svg>'
    )
...
def _social_buttons_html(size: str = "normal") -> str:
    ...
    icon_size = 18 if size == "normal" else 15
    items = []
    for name, url in SOCIAL_LINKS.items():
        if not url:
            continue
        icon = _social_icon_svg(name, icon_size)
        items.append(
